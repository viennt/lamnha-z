	...
	foreach ($associations as $type => $data) {
		foreach ($data as $alias => $details) {
			if ($details['controller'] != $this->name && !in_array($details['controller'], $done)) {
				...
				echo "\tactions += '<li>';\n";
				echo "\tactions += '<?php echo \$this->Html->link(\"<i class=\\\"fa fa-circle-o text-aqua\\\"></i> <span>List " . Inflector::humanize(Inflector::underscore($alias)) . "</span>\", array(\"controller\" => \"{$details['controller']}\", \"action\" => \"add\"), array(\"escape\" => false)); ?>';\n";
				echo "\tactions += '</li>';\n";
